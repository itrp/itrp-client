...
describe Itrp::Response do
  before(:each) do
    ...
    stub_request(:get, 'https://secret:x@api.itrp.com/v1/me').to_return(body: @person_hash.to_json)
    ...
    stub_request(:get, 'https://secret:x@api.itrp.com/v1/people').to_return(body: @people_array.to_json)
  ...
  context 'json/message' do
    ...
    it 'should add a message if the body is empty' do
      stub_request(:get, 'https://secret:x@api.itrp.com/v1/organizations').to_return(status: 429, body: nil)
    ...
    it 'should add a message if the HTTP response is not OK' do
      stub_request(:get, 'https://secret:x@api.itrp.com/v1/organizations').to_return(status: 429, body: {message: 'Too Many Requests'}.to_json)
      response = @client.get('organizations')
    ...
    it 'should add a message if the JSON body cannot be parsed' do
      stub_request(:get, 'https://secret:x@api.itrp.com/v1/organizations').to_return(body: '==$$!invalid')
      response = @client.get('organizations')
  ...
  context 'throttled?' do
    ...
    it 'should check the return message' do
      stub_request(:get, 'https://secret:x@api.itrp.com/v1/organizations').to_return(status: 500, body: {message: 'Too Many Requests'}.to_json )
      response = @client.get('organizations')
      expect(response.throttled?).to be_truthy
    end
  end
